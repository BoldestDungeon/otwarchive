...
module CssCleaner
  ...
  CSS_VARIABLE_REGEX = Regexp.new("--[a-z0-9_\\-]+", Regexp::IGNORECASE)
  VAR_FUNCTION_REGEX = Regexp.new("var\\(#{CSS_VARIABLE_REGEX}\\)", Regexp::IGNORECASE)
  UNITS_REGEX = Regexp.new("deg|cm|em|ex|in|mm|pc|pt|px|s|%", Regexp::IGNORECASE)
  NUMBER_REGEX = Regexp.new("-?\\.?\\d{1,3}\\.?\\d{0,3}|#{VAR_FUNCTION_REGEX}")
  NUMBER_WITH_UNIT_REGEX = Regexp.new("#{NUMBER_REGEX}\s*#{UNITS_REGEX}?\s*,?\s*")
  PREFIX_REGEX = Regexp.new("moz|ms|o|webkit")
  
  # CSS calc() function can take multiple numeric values separated by a valid mathematical operation
  CALC_NAME_REGEX = Regexp.new("calc", Regexp::IGNORECASE)
  CALC_OPERATIONS_REGEX = Regexp.new("[\\+\\-\\*\\/]")
  CALC_FORMULA_REGEX = Regexp.new("(?:#{NUMBER_WITH_UNIT_REGEX}\\s*#{CALC_OPERATIONS_REGEX}\\s*)+#{NUMBER_WITH_UNIT_REGEX}")
  CALC_FUNCTION_REGEX = Regexp.new("#{CALC_NAME_REGEX}\\(\\s*#{CALC_FORMULA_REGEX}\\s*\\)")
  
  # Any function that can take a number can also accept a calculation
  PAREN_NUMBER_REGEX = Regexp.new("\\(\\s*(?:#{NUMBER_WITH_UNIT_REGEX}|#{CALC_FUNCTION_REGEX})")
  ...
  VALUE_REGEX = Regexp.new("#{TRANSFORM_FUNCTION_REGEX}|#{URL_FUNCTION_REGEX}|#{COLOR_STOP_FUNCTION_REGEX}|#{COLOR_REGEX}|#{NUMBER_WITH_UNIT_REGEX}|#{ALPHA_REGEX}|#{SHAPE_FUNCTION_REGEX}|#{FILTER_FUNCTION_REGEX}|#{DROP_SHADOW_FUNCTION_REGEX}|#{VAR_FUNCTION_REGEX}|#{CALC_FUNCTION_REGEX}")
  ...
  def clean_css_code(css_code, options = {})
    return "" unless css_code.match(/\w/) # only spaces of various kinds
  ...
  def sanitize_css_property(property)
    (is_legal_property(property) || is_legal_shorthand_property(property)) ? property : ""
  end
  ...
  def sanitize_css_declaration_value(property, value)
    ...
    if property == "font-family"
      if sanitize_css_font(value).present?
  ...
  def sanitize_css_gradient(value)
    if value.match(/^([a-z\-]+)\((.*)\)/)
      ...
      interior = Regexp.last_match(2)
      cleaned_interior = tokenize_and_sanitize_css_value(interior)
      return "#{function}(#{cleaned_interior})" if function.match(/gradient/) && cleaned_interior.present?
    end
    ""
  end
